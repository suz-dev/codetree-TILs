		...
		for(int i = 1; i <= Integer.parseInt(orders[Q][1]); i++) {	// 전체 time 
			...
			if(i == time) {				
				if(code == 100) {	// new susi					
					...
					// eat susi only me
					if(customerInfo.containsKey(name) && susiInfo.containsKey(name)) {
						int[] customer = customerInfo.get(name);	// x, n
						
						if(susiInfo.get(name).containsKey(customer[0])) {
							int susiCnt = susiInfo.get(name).get(customer[0]);
							eatSusiOnlyMe(name, customer[0], customer[1], susiCnt);
						}
					}
				}
				...
				if(code == 300) {	// photo
					int leftSusi = 0;
					if(!susiInfo.isEmpty()) {
						for(HashMap<Integer, Integer> susi : susiInfo.values()) {
							for(Integer tmpSusi : susi.values()) {
								leftSusi += tmpSusi;
							}
						}
					}
	...
	public static void eatSusi() {
		...
		for(String name : customerInfo.keySet()) {
			...
			if(val[1] <= 0) {
				...
				susiInfo.get(name).remove(val[0]);
			}
		}
	}
